...
Lemma negb_twice : forall b : bool, negb (negb b) = b.
Proof.
  intros [].
  - simpl. reflexivity.
  - simpl. reflexivity.
...
Proof.
  induction n as [|n'].
  - simpl. reflexivity.
  - rewrite -> IHn'. simpl. rewrite -> negb_twice. reflexivity.
Qed.
(** [] *)
...
(* Answer: The induction will add incuction assumption to the context
...
(*
  ...
  - rewrite <- mult_n_Sm. rewrite -> IHp'.
    ...
    { rewrite -> plus_comm.  rewrite -> plus_swap. rewrite -> plus_assoc. reflexivity. }
    rewrite -> H. rewrite -> mult_n_Sm. rewrite <- plus_assoc. rewrite -> mult_n_Sm.
    reflexivity.
Qed.
*)
...
Fixpoint bin_to_nat (m:bin) : nat :=
  ...
  | B n => 1 + (bin_to_nat n) +  (bin_to_nat n)
  end.
...
Proof.
  ...
  - simpl. reflexivity.
  - simpl. reflexivity.
  - simpl. rewrite -> IHb2. simpl. rewrite -> plus_comm.
    simpl. reflexivity.
Qed.
...
(*
  According to the bin definition, it allows innormal binary 0 form,
  eg. Z expresses binary 0, A Z, A (A Z), A (A (A Z)), ... express binary 0 too.
  So for inmormal binary 0, eg A Z:
      bin_to_nat (A Z) = O
      nat_to_bin O = Z
      Z is not reflexivity with (A Z).
 *)
...
Lemma double_nat_eq_A_bin : forall (n : nat),
    nat_to_bin (S n + S n) = A (nat_to_bin (S n)).
Proof.
  induction n as [|n'].
  - simpl. reflexivity.
  - simpl. rewrite <- plus_n_Sm. rewrite -> plus_comm.
    rewrite -> plus_n_Sm. rewrite -> IHn'.
    simpl. reflexivity.
Qed.

Theorem bin_nat_bin_qe_nb : forall b : bin,
    nat_to_bin (bin_to_nat b) = normalize b.
Proof.
  induction b as [|b1|b2].
  - simpl. reflexivity.
  - simpl. destruct (bin_to_nat b1) as [|n].
    + rewrite <- IHb1. simpl. reflexivity.
    + rewrite <- IHb1. rewrite -> double_nat_eq_A_bin. simpl.
      destruct (nat_to_bin n) as [|m1|m2].
      * simpl. reflexivity.
      * simpl. reflexivity.
      * simpl.  reflexivity.
  - simpl. destruct (bin_to_nat b2) as [|n].
    + rewrite <- IHb2. simpl. reflexivity.
    + rewrite <- IHb2. rewrite -> double_nat_eq_A_bin.
      simpl. reflexivity.
Qed.
