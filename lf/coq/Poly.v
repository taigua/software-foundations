...
Theorem app_assoc : forall A (l m n:list A),
  l ++ m ++ n = (l ++ m) ++ n.
Proof.
  intros A l m n. induction l as [| h t].
  - simpl. reflexivity.
  - simpl. rewrite -> IHt. reflexivity.
Qed.
...
Proof.
  intros X l1 l2. induction l1 as [|h t].
  - simpl. reflexivity.
  - simpl. rewrite -> IHt. reflexivity.
Qed.
(** [] *)
...
Proof.
  induction l as [|h t].
  - simpl. reflexivity.
  - simpl. rewrite -> map_app_distr. rewrite <- IHt. reflexivity.
Qed.
(** [] *)
...
(* Check whether a list of nat is all even:
   fold evenb [1; 2; 3; 4] true
*)
...
(* Let me try a poly full constfun *)
...
(** **** Exercise: 2 stars, advanced (nth_error_informal)
   ...
   Proof:
     ...
     For n = 0:
        ...
        length l = 0 implies l is [], so @nth_error X l 0 = None

     For other n:

        assuming length l = n and @nth_error X l n = None

     Then, for S n:

        @nth_error X l (S n) = @nth_error X l' (pred (S n)) where l = a :: l'

                             = @nth_error X l' n

        According to length l = S n, l = a :: l', so length l' = n, so
