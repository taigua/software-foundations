...
Proof.
  ...
  - intros m H. destruct  m as [| m'].
    ...
    + simpl in H. rewrite <- plus_n_Sm in H. rewrite <- plus_n_Sm in H.
      injection H as H1.  apply IHn' in H1. rewrite -> H1. reflexivity.
Qed.
(** [] *)
...
Proof.
  ...
  - intros m H. destruct m as [| m'].
    + discriminate.
    + simpl in H. apply IHn' in H. apply f_equal.
      apply H.
Qed.
(** [] *)
...
(** **** Exercise: 2 stars, advanced (eqb_true_informal)
    ...
    Give a careful informal proof
    of [eqb_true], being as explicit
    as possible about quantifiers. *)

(*
  Proof: Let n be a nat. We prove by induction on n that, for any n,
         if n =? m = true then n = m.

    - First, suppose n = 0, and suppose m is a number such that n =? m = true.
      We must show that m = 0.

      There are two cases to consider for m. If m = 0 we are done, since n = 0
      = m, as required. Otherwise, if m = S m' for some m', we derive a contradiction: by the hypothesis we know n =? S m' = true, but this contradicts the assumption that n = 0.

    - Second, suppose n = S n' and that m is again a number such that n =? m = true. We must show that S n' = m, with the induction hypothesis that for every
    number s, if S n' =? s = true then S n' = s. There are two cases for m.

      If m = 0, then S n' =? 0 = true, a contradiction.

      Thus, we may assume that m = S m' for some m'.  Instantiating the
      induction hypothesis with m' thus allows us to conclude that n' = m',
      and it follows immediately that S n' = S m'. Since S n' = n and S m' = m,
      this is just what we wanted to show.
 *)

(* Do not modify the following line: *)
Definition manual_grade_for_informal_proof : option (nat*string)
...
Lemma split_lemma : forall X Y (l : list (X * Y)) l1 l2 x y,
    split l = (l1, l2) -> split ((x, y)::l) = (x::l1, y::l2).
Proof.
  intros X Y l l1 l2 x y H.
...
Proof.
  ...
  - intros l1 l2 H. injection H as H1 H2.
    rewrite <- H1. rewrite <- H2. reflexivity.
  - intros l1 l2 H. simpl in H. destruct (split t) as (lx, ly).
    injection H as H1 H2. rewrite <- H1. rewrite <- H2. simpl.
    assert (combine lx ly = t). { apply IHt. reflexivity. }
                                rewrite -> H. reflexivity.
Qed.
(** [] *)
...
Proof.
  intros n. induction n as [| n'].
  - intros m p H1 H2. destruct m as [| m'].
    + apply H2.
    + discriminate H1.
  - intros m. destruct m as [| m'].
    ...
    + intros p. destruct p as [| p'].
      ...
      * intros H1 H2. simpl in H1. simpl in H2. simpl.
        apply IHn' in H2. apply H2. apply H1.
Qed.
(** [] *)
...
Proof.
  ...
  - intros l2 H. destruct l2 as [| h2 t2].
    + discriminate H.
    + simpl in H. injection H as H'.  apply IHt1 in H'.
      simpl. rewrite -> H'. reflexivity.
Qed.
(* Do not modify the following line: *)
...
Theorem filter_exercise : forall (X : Type) (test : X -> bool)
     ...
     test x = true.
Proof.
  intros X test x l. induction l as [| h t].
  - intros lf H. simpl in H. discriminate H.
  - intros lf H. simpl in H. destruct (test h) eqn: TH.
    + injection H as H'. rewrite <- H'. apply TH.
...
Proof.
  intros X test l. induction l as [| h t].
  - reflexivity.
  - simpl. destruct (test h) eqn: TH.
    + unfold existsb'. simpl. rewrite -> TH. simpl. reflexivity.
    + unfold existsb'. simpl. rewrite -> TH. simpl. unfold existsb' in IHt.
      apply IHt.
Qed.
